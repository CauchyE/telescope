...
export class CosmosSDKService {
  ...
  constructor() {
    ...
    this.restURL$ = new BehaviorSubject('')
    this.websocketURL$ = new BehaviorSubject('')
    this.chainID$ = new BehaviorSubject('')
    this.sdk$ = combineLatest([this.restURL$, this.websocketURL$, this.chainID$]).pipe(map(([restURL, websocketURL, chainID]) => ({
